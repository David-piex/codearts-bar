...
- JetBrains 查询 bundle 与导出 bundle 分离，日常查询 bundle（含数据库健康和失败脱敏）由小于 126000 字节的门禁约束，当前为 124941 字节。
...
桌面端、VS Code 和 JetBrains 都能从会话列表或会话详情导出单个会话。桌面端额外支持批量导出。
...
- Desktop、VS Code、JetBrains 和 CLI 均已接入单会话导出，Desktop 另有批量导出；批量 Requests 明细包含错误类型。
...
质量门禁已恢复全绿。后续新增协议、插件能力和导出格式都必须增加对应回归测试；预算调整必须记录明确的产物归因，且不能放宽无关热路径门禁。P2 因独立 XLSX 引擎将 JetBrains 发布 ZIP 上限从 550000 调整为 950000 字节，当前 `1.16.35` 最终产物为 879590 字节；查询 bundle 因加入 diagnostics 数据库统计与递归脱敏，将上限从 125000 调整为 126000 字节，当前为 124941 字节。Renderer 的 330000 字节上限保持不变。
