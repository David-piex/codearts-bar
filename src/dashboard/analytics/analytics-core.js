...
function normalizeUsageMetric(st = {}){
  return {
    ...
    promptTokens: Number(st.input || 0) + Number(st.cacheRead || 0),
    outputTokens: Number(st.output || 0) + Number(st.reasoning || 0),
    consumedTokens: Number(st.input || 0) + Number(st.cacheRead || 0) + Number(st.output || 0) + Number(st.reasoning || 0),
