...
function renderChart(rows, s){
  ...
  const seriesButtons = SERIES
    ...
    .map((cfg) => `<button class="series-chip ${visibleSeries.has(cfg.key) ? 'active' : ''}" data-series="${cfg.key}" aria-pressed="${visibleSeries.has(cfg.key) ? 'true' : 'false'}"><i style="background:${cfg.color}"></i>${esc(cfg.label)}</button>`).join('');
