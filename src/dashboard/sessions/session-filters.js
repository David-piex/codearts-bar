...
function sortSessions(list){ const arr = [...list]; const score = (x) => x.usage || {}; arr.sort((a, b) => { const pa = isPinnedSession(a) ? 1 : 0; const pb = isPinnedSession(b) ? 1 : 0; if(pa !== pb) return pb - pa; let result = 0; if(sessionSort === 'opportunity') result = cacheOpportunityScore(score(b)) - cacheOpportunityScore(score(a)) || (score(b).total || 0) - (score(a).total || 0); else if(sessionSort === 'token') result = (score(b).total || 0) - (score(a).total || 0); else if(sessionSort === 'turns') result = (score(b).userTurns || 0) - (score(a).userTurns || 0); else if(sessionSort === 'cache') result = (cacheHitRate(score(b)) || -1) - (cacheHitRate(score(a)) || -1) || (score(b).total || 0) - (score(a).total || 0); else result = (b.updatedAt || 0) - (a.updatedAt || 0); return result || sessionKeyFor(a).localeCompare(sessionKeyFor(b)); }); return arr; }
...
function sessionSortHtml(){ const opts = [['updated', TXT.byUpdated], ['token', TXT.byToken], ['turns', TXT.byTurns]]; if(!opts.some(([k]) => k === sessionSort)){ sessionSort = 'updated'; persistStateNow('sessionSort', sessionSort); } return `<div class="select session-sort"><span>${TXT.sort}</span><select data-select="sessionSort">${opts.map(([k, label]) => `<option value="${k}" ${sessionSort === k ? 'selected' : ''}>${label}</option>`).join('')}</select></div>`; }
...
function sessionProjectFilterHtml(s){
  ...
  if(sessionProjectFilter !== 'all' && !projects.some((p) => p.key === sessionProjectFilter)){ sessionProjectFilter = 'all'; persistStateNow('sessionProjectFilter', sessionProjectFilter); }
...
function sessionQuickMatches(session, key = sessionQuickFilter){
  ...
  if(key === 'recent') return ((typeof rendererNow === 'function' ? rendererNow() : Date.now()) - Number(session.updatedAt || 0)) <= 7 * 86400000;
  return true;
}
