...
function periodSparkline(stat){
  const bars = [['input', COLORS.input], ['output', COLORS.output], ['cacheWrite', COLORS.cacheWrite], ['cacheRead', COLORS.cacheRead]];
  const max = Math.max(1, ...bars.map(([key]) => Number(stat?.[key] || 0)));
  return `<div class="period-sparkline" aria-hidden="true" style="display:flex;align-items:end;gap:3px;height:18px;margin-top:8px;padding-top:3px;border-top:1px solid var(--mac-line-soft)">${bars.map(([key, color]) => `<i style="flex:1;min-width:3px;height:${Math.max(3, Math.round(Number(stat?.[key] || 0) / max * 100))}%;border-radius:2px 2px 1px 1px;background:${color};opacity:.7"></i>`).join('')}</div>`;
}
function periodCard(label, stat, tone){ return `<div class="period-card ${tone}"><div class="period-label">${esc(label)}</div><div class="period-value">${compact(stat.total || 0)}</div><div class="period-sub">${n(stat.messages || stat.requests || 0)} ${TXT.requests}</div>${periodSparkline(stat)}</div>`; }
...
function renderCompactMenu(s, rows){
  ...
  return `<div class="compact-stage single"><section class="codex-compact-card compact-single-card"><div class="compact-provider"><div class="logo compact-logo"><img src="../assets/codearts-logo-ui.png" width="29" height="29" decoding="async" draggable="false" /></div><div><h2>${TXT.compactTitle}</h2><p>${TXT.compactHint}</p></div><span>${TXT.realtime}</span></div>${compactSourcePill(s)}${compactControlsHtml()}${compactPaneTabs()}<div class="compact-pane">${panes[compactPane] || panes.overview}</div></section></div>`;
}
