...
public record UsageSnapshot(
    ...
    private static List<RequestInfo> requests(JsonArray a) { List<RequestInfo> out=new ArrayList<>(); for(JsonElement e:a){if(!e.isJsonObject())continue;JsonObject x=e.getAsJsonObject();out.add(new RequestInfo(string(x,"id"),string(x,"sessionTitle"),first(string(x,"source"),string(x,"sourceLabel")),string(x,"provider"),string(x,"model"),numberEither(x,"time","createdAt"),integerOrNull(x,"status"),bool(x,"ok",true),string(x,"error"),number(x,"total"),number(x,"input"),number(x,"output"),number(x,"reasoning"),number(x,"cacheRead"),number(x,"cacheWrite"),number(x,"latencyMs"),decimalOrNull(x,"ttftMs"),decimalOrNull(x,"firstContentMs"),decimalOrNull(x,"outputTokensPerSec")));}return List.copyOf(out); }
