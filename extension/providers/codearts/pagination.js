...
function getRequestsPageNative(payload = {}) {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      const rawMessages = nativeAllParams(db, `select id, session_id, time_created, time_updated, data from message where ${where} order by time_created desc limit ? offset ?`, [...params, singleSource ? limit : offset + limit, singleSource ? offset : 0]);
  ...
  const sorted = rowsBySource.sort((a, b) => (b.time || 0) - (a.time || 0));
  const items = singleSource ? sorted.slice(0, limit) : sorted.slice(offset, offset + limit);
...
async function getRequestsPageSqlJs(payload = {}) {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      total += Number(count || 0);
      const rawMessages = sqlJsAllParams(db, `select id, session_id, time_created, time_updated, data from message where ${where} order by time_created desc limit ? offset ?`, [...params, singleSource ? limit : offset + limit, singleSource ? offset : 0]);
      const messages = tagRows(rawMessages, source);
...
function getSessionsPageNative(payload = {}) {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      total += Number(count || 0);
      const rawSessions = nativeAllParams(db, `select id, title, directory, version, time_created, time_updated, time_archived from session where ${where} order by time_updated desc limit ? offset ?`, [...params, singleSource ? limit : offset + limit, singleSource ? offset : 0]);
  ...
  const sorted = rowsBySource.sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0));
  const items = singleSource ? sorted.slice(0, limit) : sorted.slice(offset, offset + limit);
...
async function getSessionsPageSqlJs(payload = {}) {
  ...
  for (const source of sources) {
    ...
    try {
      ...
      total += Number(count || 0);
      const rawSessions = sqlJsAllParams(db, `select id, title, directory, version, time_created, time_updated, time_archived from session where ${where} order by time_updated desc limit ? offset ?`, [...params, singleSource ? limit : offset + limit, singleSource ? offset : 0]);
      const sessions = tagRows(rawSessions, source);
