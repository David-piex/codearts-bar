...
const direct = spawnSync(process.execPath, [bin, "self-test"], { cwd: root, encoding: "utf8", timeout: 30000 });
...
try {
  ...
  const packed = spawnSync(process.execPath, [npmCli, "pack", "--json", "--pack-destination", temp], { cwd: root, encoding: "utf8", timeout: 120000, windowsHide: true });
  ...
  const extract = spawnSync("tar.exe", ["-xf", tarball, "-C", unpacked], { encoding: "utf8", timeout: 30000, windowsHide: true });
  ...
  const packedRun = spawnSync(process.execPath, [packedBin, "self-test"], { cwd: path.join(unpacked, "package"), encoding: "utf8", timeout: 30000 });
