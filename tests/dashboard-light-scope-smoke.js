...
const { buildDashboardLightPair } = require('../src/main/dashboard-light');
...
async function main(){
  ...
  try {
    ...
    for(const snap of [fullSnap, dashboardSnap]){
      ...
      assert.deepEqual(snap.queryScope, { source: 'all', model: 'all', rangeKey: '', start: 0, end: 0 });
      assert.deepEqual(snap.usageScope, { source: 'all', model: 'all', rangeKey: '', start: 0, end: 0 });
    }
  } finally {
