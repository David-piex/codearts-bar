...
assert.doesNotMatch(source, /getSnapshotWithCache|collectRows/);
...
(async () => {
  ...
  const summary = await data.getExtensionSummary({});
  ...
  const hotSummary = await data.getExtensionSummary({});
  ...
  const details = await data.getExtensionDetails({});
