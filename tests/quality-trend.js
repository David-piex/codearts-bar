...
const results = checks.map(([name, actual, operator, expected]) => ({ name, actual: Number(actual), operator, expected, ok: operator === '<=' ? Number(actual) <= expected : Number(actual) >= expected }));
const trend = { schemaVersion: 1, generatedAt: new Date().toISOString(), version: metrics.version, baseline: l, results, ok: results.every((item) => item.ok) };
