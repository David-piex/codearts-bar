...
async function main() {
  ...
  const metrics = {
    ...
    bundles: { rendererBytes: size('src/dashboard-renderer.js'), cssBytes: size('src/dashboard-bundle.css'), jetbrainsZipBytes: (() => { const name = `codearts-bar-jetbrains-${require('../package.json').version}.zip`; try { return fs.statSync(path.join(root, 'jetbrains-plugin/build/distributions', name)).size; } catch { return 0; } })() },
